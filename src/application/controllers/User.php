...
class User extends CI_Controller{
	...
	public function edit($id){
		...
		if($id){
			...
			$this->form_validation->set_rules('id_divisi','Divisi');
			$this->form_validation->set_rules('id_jabatan','Jabatan');
			$this->form_validation->set_rules('id_golongan','Golongan');
			$this->form_validation->set_rules('id_jam_kerja','Jam Kerja');
			...
			if($this->form_validation->run()==FALSE){
				...
				if($count > 0){
					...
					//load select box divisi
					$div = $this->Divisi_m->get_all(false)->result_array();
					$data['divisi'] = '<select name="id_divisi" id="id_divisi" class="form-control">';
					$data['divisi'] .= '<option value="">Pilih Divisi</option>';
					foreach($div as $d){
						$selected = '';
					...
					//load select box jabatan
					$jab = $this->Jabatan_m->get_all(false)->result_array();
					$data['jabatan'] = '<select name="id_jabatan" id="id_jabatan" class="form-control">';
					$data['jabatan'] .= '<option value="">Pilih Jabatan</option>';
					foreach($jab as $j){
						$selected = '';
					...
					//load select box golongan
					$gol = $this->Golongan_m->get_all(false)->result_array();
					$data['golongan'] = '<select name="id_golongan" id="id_golongan" class="form-control"';
					$data['golongan'] .= '<option value="">Pilih Golongan</option>';
					foreach($gol as $g){
						$selected = '';
